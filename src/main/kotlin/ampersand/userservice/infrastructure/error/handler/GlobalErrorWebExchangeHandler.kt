...
class GlobalErrorWebExchangeHandler(
    ...
    private fun handleError(request: ServerRequest): Mono<ServerResponse> =
        when(val throwable = super.getError(request)) {
            is MemberException -> buildErrorResponse(throwable)
            is ConnectException -> buildErrorResponse(MemberException("Cannot Connect to Service", HttpStatus.INTERNAL_SERVER_ERROR))
            else -> buildErrorResponse(MemberException("Internal Server Error", HttpStatus.INTERNAL_SERVER_ERROR))
        }
